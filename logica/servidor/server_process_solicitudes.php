...
$sTabla = "solicitud AS solicitud
JOIN conductor AS c ON c.id_conductor = solicitud.id_conductor
JOIN pasajero AS p ON p.id_pasajero = solicitud.id_pasajero
JOIN persona AS pasajero ON pasajero.id_persona = p.id_persona
JOIN persona AS conductor ON conductor.id_persona = c.id_persona
JOIN vehiculo AS vehiculo ON vehiculo.id_vehiculo = c.id_vehiculo
JOIN localizacion AS localizacion ON localizacion.id_localizacion = solicitud.id_localizacion";
$sTabla2 = "solicitud AS solicitud
JOIN conductor AS c ON c.id_conductor = solicitud.id_conductor
JOIN pasajero AS p ON p.id_pasajero = solicitud.id_pasajero
JOIN persona AS pasajero ON pasajero.id_persona = p.id_persona
JOIN persona AS conductor ON conductor.id_persona = c.id_persona
...
$aColumnas = array(
    ...
    'pasajero.nombres AS nombrepasajero', 
    'pasajero.telefono AS telefonopasajero', 
    'conductor.nombres AS nombreconductor', 
    'CONCAT("N°: ", vehiculo.unidad, " - Marca: ", vehiculo.marca, " Color: ", vehiculo.color, " - Placa: ", vehiculo.placa, " ") AS unidadconductor', 
    'CONCAT("Origen: ", localizacion.direccion_actual, "  Destino: ", localizacion.direccion_destino, " ") AS direcciones', 
...
$aColumnas3 = array('id_solicitud', 'pasajero.nombres', 'pasajero.telefono','solicitud.estado','conductor.nombres','referencia ', 'localizacion.direccion_actual', 'localizacion.direccion_destino', 'precio', 'fechayhora', 'tipo', 'vehiculo.unidad', 'vehiculo.marca', 'vehiculo.color', 'vehiculo.placa', 'tipo_pago', 'localizacion.direccion_actual');
...
while ($aRow = $rResult->fetch_array()) {
	...
	$row[] = "<td class='text-center'><button type='button' onclick='finalizarsol(" . $aRow['id_solicitud'] . ")' class='boton-save'  data-toggle='modal' data-target='#modal-notificationes'  style='width: 30px;height: 30px;margin: 0 0 0 0;padding: 0 0 0 0;'><i class='fas fa-exclamation-triangle'></i></button></td>";
	$output['aaData'][] = $row;
}
