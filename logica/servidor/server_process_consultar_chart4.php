...
global $db;
$query = $db->query("SELECT `fecha` FROM `operaciones` WHERE DATE_FORMAT(fecha, '%Y/%m/%d') BETWEEN '{$mes_anterior}' AND '{$fechaactual}' GROUP by fecha ORDER BY `operaciones`.`fecha` DESC");
$fecha_array = array();
$efectivo = array();
$billetera = array();
$banca = array();
