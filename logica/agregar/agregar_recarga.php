...
$query_saldo_conductor = $db->query("SELECT id_saldo FROM `saldo_conductor` WHERE `id_conductores` = {$id_conductores} AND `estado`= 1");
$id_saldo_conductor = $query_saldo_conductor->fetch_assoc();
$id_saldo_conductors = $id_saldo_conductor['id_saldo'];
$query  = "UPDATE `saldo_conductor` SET ";
$query .= "estado='0'";
$query .= "WHERE id_saldo='{$id_saldo_conductors}'";
if ($db->query($query)) {
    ...
    if ($db->query($query2)) {
        ...
        VALUES ('{$monto}','Recarga de saldo','{$tipo_opreacion}','{$metodo_pago}','{$tipo_caja}','{$tipo_pago}','1','{$detalleact}','{$complemento}','{$_SESSION["userId"]}','{$fecha}','{$hora24}')";
        if ($db->query($query4)) {
            echo $response;
        }
    } else {
