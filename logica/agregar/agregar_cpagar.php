...
$query  = "UPDATE `otrosegresos` SET `estado_deuda` = '0' WHERE `otrosegresos`.`id` = {$id};";
if ($db->query($query)) {
    $query2 = "INSERT INTO `otrosegresos`(`detalle`, `monto`, `fecha_hora`, `fecha_reg`, `monto_anterior`, `deuda`, `aporte`,`precio`, `concepto`, `id_cliente`, `nombre_cliente`, `documento_cliente`, `estado`, `usuario`, `nombre`, `estado_deuda`) 
VALUES ('{$detalle}','{$monto}','{$fechahora}','{$f_reg}','{$monto}','{$deuda}','{$importe}','{$precio}','{$concepto}',0,'{$nombrecl}','{$doc}',1,'{$_SESSION["userId"]}','{$_SESSION["nombre_usuario"]}','{$estado_deuda}')";
    //echo $query2;
    if ($db->query($query2)) {
        ...
        VALUES ('{$importe}','Egreso pago de deuda','{$tipo_opreacion}','{$metodo_pago}','{$tipo_caja}','{$tipo_pago}','1','{$detalleact}','{$complemento}','{$_SESSION["userId"]}','{$fecha}','{$hora24}','egreso')";
        //echo $query4;
        if ($db->query($query4)) {
            echo 'ok';
        }
