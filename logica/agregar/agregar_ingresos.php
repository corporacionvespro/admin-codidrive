...
//echo $query2;
if ($db->query($query2)) {
    $query_s = $db->query("SELECT id FROM `otrosingresos` ORDER BY `id` DESC LIMIT 1;");
    //echo $query_s;
    ...
    $query4 = "INSERT INTO `operaciones`
        ...
        VALUES ('{$importe}','Otro ingreso','{$tipo_opreacion}','{$metodo_pago}','{$tipo_caja}','{$tipo_pago}','1','{$detalleact}','{$complemento}','{$_SESSION["userId"]}','{$fecha}','{$hora24}')";
    //echo $query4;
    if ($db->query($query4)) {
        echo 'ok';
    }
