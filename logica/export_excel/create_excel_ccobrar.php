...
header("Content-Disposition: attachment; filename=CUENTAS_POR_COBRAR_" . date('Y:m:d:m:s') . ".xls");
...
if ($_GET['sSearch'] != "") {
	$sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumnas)) . " FROM $sTabla $sWhere AND estado_deuda = '1' $sOrder";
} else {
	$sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumnas)) . " FROM $sTabla $sWhere where estado_deuda = '1' $sOrder";
}
