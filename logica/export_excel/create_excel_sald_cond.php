...
header("Content-Disposition: attachment; filename=SALDO_DE_CONDUCTORES_" . date('Y:m:d:m:s') . ".xls");
...
if ($_GET['sSearch'] != "") {
    $sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumnas)) . " FROM $sTabla $sWhere AND conductor.estado <>'sinaprobar' AND saldo_conductor.estado=1 $sOrder";
} else {
    $sQuery = "SELECT SQL_CALC_FOUND_ROWS " . str_replace(" , ", " ", implode(", ", $aColumnas)) . " FROM $sTabla $sWhere AND conductor.estado <>'sinaprobar' AND saldo_conductor.estado=1 $sOrder";
}
//echo $sQuery;
...
while ($aRow = $rResult->fetch_array()) {
    ...
    for ($i = 0; $i < count($aColumnas2); $i++) {
        if ($aColumnas2[$i] == "id_conductor") {
            ...
            //$row[] = "<td class='text-center'><img src='https://codidrive.com/codiapi/images/" . $aRow[$aColumnas2[$i]] . "' class='circle' style='width: 70px;height: 70px;overflow: hidden;border-radius: 45px;border: none;text-align: center;object-fit: cover;color: transparent;text-indent: 10000px;'></td>";
