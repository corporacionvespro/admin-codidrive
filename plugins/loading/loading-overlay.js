...
(function ($) {
  ...
  var methods = {
    init: function (options) {
	  ...
	  if(opts.tipo != undefined ){
		if(opts.tipo=='input'){
		  opts.spinnerClass='loading-spinner-input';
		  opts.iconClass='loading-icon-input';
		}
	  }
	...
	//target.find('.' + opts.overlayClass).detach();
	  if($("#"+target.attr('id')+" div."+opts.overlayClass).length>0){
		$("#"+target.attr('id')+" div."+opts.overlayClass).first().detach();
	  }
	  ...
	  if (target.hasClass(opts.loadingClass)) {
		  ...
		  target.find('.' + opts.loadingClass).first().removeClass(opts.loadingClass);
      }
      return target;
    },
  ...
  $.fn.loadingOverlay.defaults = {
    ...
    loadingText: 'Cargando'            // Text within loading overlay
  };
